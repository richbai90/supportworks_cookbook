...
action :migrate do
  ...
  execute 'dump_data' do
    ...
    not_if { ::File.exists?(new_resource.host.to_s) }
  end
  ...
  execute 'restore_data' do
    ...
    cwd mysql_bin
    command "mysql -u root --port 5002 -f < #{::File.join(file_path, 'dump.sql')}"
    ignore_failure true
  ...
  execute 'restore_analyst_data' do
    file_path = ::File.exists?(new_resource.host.to_s) ? new_resource.host.to_s : Chef::Config['file_cache_path']
    cwd mysql_bin
    command "mysql -u root --port 5002 -f < #{::File.join(file_path, 'analyst_dump.sql')}"
    ignore_failure true
    only_if "cd #{'"' + mysql_bin + '"'} && mysql -u root --port 5002"
    not_if { new_resource.host === :no_migrate.to_s }
  end
