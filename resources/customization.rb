        ...
        setup['prereq'].each do |prereq|
          ruby_block 'wait for ' + prereq do
            block do
              ...
              backup_and_copy(File.join(::Dir.pwd, d["package"]), swserver, core_services, ::File.join(mysql_path, 'bin'), swdata_db_user || cache_db_user, swdata_db_password || cache_db_password)
